...
function Nsh(stdio, completer, terminal)
{
  ...
  this.prompt = function(smallPrompt)
  {
    ...
    this.clearLine()  // TODO Is this needed for builtins? We should remove it
...
Nsh.eval = function(stdio, line, callback)
{
  ...
  {
    if(error.constructor !== parse.SyntaxError) return callback(error)

    callback(new SyntaxError('end of file unexpected'))
  }
